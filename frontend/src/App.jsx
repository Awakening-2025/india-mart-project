...
import React, { useState, useEffect, useRef } from 'react'; // <<-- useEffect aur useRef import karein
import Header from './components/Header';
import Sidebar from './components/Sidebar';
import ProductCard from './components/ProductCard';
import Footer from './components/Footer';
...
function App() {
  // Baaki saare states waise hi rahenge
  const [selectedCategory, setSelectedCategory] = useState('All Categories');
  const [viewMode, setViewMode] = useState('grid');
  const [sortBy, setSortBy] = useState('relevance');
  const [isAuthFormOpen, setIsAuthFormOpen] = useState(false);
  ...
  // 1. STICKY HEADER KE LIYE STATES AUR REFS
  const [isSticky, setIsSticky] = useState(false);
  const [headerPlaceholderHeight, setHeaderPlaceholderHeight] = useState(0);
  const topBarRef = useRef(null);
  const mainHeaderRef = useRef(null);

  // 2. SCROLL LISTENER KE LIYE useEffect
  useEffect(() => {
    // Ye function har scroll par chalega
    const handleScroll = () => {
    ...
    // Main header ki height measure karo (sirf ek baar)
    const mainHeaderHeight = mainHeaderRef.current ? mainHeaderRef.current.offsetHeight : 0;
    setHeaderPlaceholderHeight(mainHeaderHeight);

    window.addEventListener('scroll', handleScroll);

    // Component unmount hone par listener ko saaf karna zaroori hai
    return () => {
      window.removeEventListener('scroll', handleScroll);
    };
  }, []); // Empty array ka matlab ye effect sirf component ke load hone par chalega

  // ... (Aapka filtering aur sorting logic yahan same rahega) ...
  const filteredProducts = selectedCategory === 'All Categories'
    ? products
    : products.filter(product => product.category === selectedCategory);
  ...
  return (
    <ToastProvider>
      <div className="min-h-screen bg-gray-50">
        ...
        {/* 3. NAYE PROPS HEADER KO PASS KAREIN */}
        <Header
          cartItemsCount={getTotalItems()}
        ...
        {/* 4. PLACEHOLDER DIV ADD KAREIN */}
        {/* Jab header sticky ho, to ye div uski jagah lega taaki content jump na kare */}
        {isSticky && <div style={{ height: `${headerPlaceholderHeight}px` }} />}
        ...
        {isAuthFormOpen && (
          <AuthForm onClose={() => setIsAuthFormOpen(false)} />
        )}
        <ToasterContainer />
      </div>
    </ToastProvider>
